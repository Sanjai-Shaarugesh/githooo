...
class o extends l.A {
	constructor(t, e) {
		super(t, e);
		const s = t.getAttribute('data-hs-stepper'),
			i = s ? JSON.parse(s) : {},
	...
	nextClick() {
		...
		if (
			...
			(0, n.JD)('beforeNext.hs.stepper', this.el, this.currentIndex),
	...
	getUncompletedSteps(t = !1) {
		return this.navItems.filter(({ isCompleted: e, isSkip: s }) =>
			t ? !e || s : !e && !s,
		);
	}
	setTotalSteps() {
	...
	buildNav() {
		this.el
			...
			.forEach((t) => this.addNavItem(t)),
			this.navItems.forEach((t) => this.buildNavItem(t));
	}
	buildNavItem(t) {
		const { index: e, isDisabled: s, el: i } = t;
		e === this.currentIndex && this.setCurrentNavItem(),
			('linear' !== this.mode || s) &&
				...
				i.addEventListener(
					'click',
					this.onNavItemClickListener.find((t) => t.el === i).fn,
				));
	}
	addNavItem(t) {
		const {
			...
			isProcessed: h = !1,
			hasError: r = !1,
		} = JSON.parse(t.getAttribute('data-hs-stepper-nav-item'));
		i && t.classList.add('success'),
	...
	setCurrentNavItem() {
		this.navItems.forEach((t) => {
			const { index: e, el: s } = t;
			e === this.currentIndex
				? this.setCurrentNavItemActions(s)
				: this.unsetCurrentNavItemActions(s);
		});
	}
	setCurrentNavItemActions(t) {
		t.classList.add('active'),
			this.fireEvent('active', this.currentIndex),
			(0, n.JD)('active.hs.stepper', this.el, this.currentIndex);
	}
	...
	buildContent() {
		this.el
			...
			.forEach((t) => this.addContentItem(t)),
	...
	addContentItem(t) {
		const {
			...
			isCompleted: i = !1,
			isSkip: n = !1,
		} = JSON.parse(t.getAttribute('data-hs-stepper-content-item'));
		i && t.classList.add('success'),
			n && t.classList.add('skipped'),
	...
	setCurrentContentItem() {
		...
		this.contentItems.forEach((t) => {
			const { index: e, el: s } = t;
			e === this.currentIndex
				? this.setCurrentContentItemActions(s)
				: this.unsetCurrentContentItemActions(s);
		});
	}
	hideAllContentItems() {
	...
	handleBackButtonClick() {
		1 !== this.currentIndex &&
			...
			(0, n.JD)('back.hs.stepper', this.el, this.currentIndex));
	}
	...
	handleNextButtonClick(t = !0) {
		...
		'linear' === this.mode && this.removeOptionalClasses(),
			...
			(0, n.JD)('next.hs.stepper', this.el, this.currentIndex);
	}
	...
	setSkipItem(t) {
		const e = this.navItems.find(
				({ index: e }) => e === (t || this.currentIndex),
			),
			s = this.contentItems.find(
				({ index: e }) => e === (t || this.currentIndex),
			);
		e && s && (this.setSkipItemActions(e), this.setSkipItemActions(s));
	}
	...
	showSkipButton() {
		...
		const { isOptional: t } = this.navItems.find(
	...
	handleSkipButtonClick() {
		this.setSkipItem(),
			...
			(0, n.JD)('skip.hs.stepper', this.el, this.currentIndex);
	}
	...
	setCompleteItem(t) {
		...
		e && s && (this.setCompleteItemActions(e), this.setCompleteItemActions(s));
	}
	...
	handleCompleteStepButtonClick() {
		this.setCompleteItem(),
			...
			(0, n.JD)('complete.hs.stepper', this.el, this.currentIndex),
	...
	handleFinishButtonClick() {
		const t = this.getUncompletedSteps(),
			...
			{ el: s } = this.contentItems.find(({ isFinal: t }) => t);
		t.length && t.forEach(({ index: t }) => this.setCompleteItem(t)),
			(this.currentIndex = this.totalSteps),
			this.setCurrentNavItem(),
			this.hideAllContentItems();
		const i = this.navItems.find(({ index: t }) => t === this.currentIndex);
		(i ? i.el : null).classList.remove('active'),
			...
			e.length <= 1 && ((this.isCompleted = !0), this.setCompleted()),
			this.fireEvent('finish', this.currentIndex),
			(0, n.JD)('finish.hs.stepper', this.el, this.currentIndex);
	}
	buildResetButton() {
		this.resetBtn &&
	...
	handleResetButtonClick() {
		this.backBtn && (this.backBtn.style.display = ''),
			...
			(0, n.JD)('reset.hs.stepper', this.el, this.currentIndex);
	}
	...
	destroy() {
		this.el.classList.remove('completed'),
			...
			this.onNavItemClickListener.length &&
				this.onNavItemClickListener.forEach(({ el: t, fn: e }) => {
